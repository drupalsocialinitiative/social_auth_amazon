 ...
 * A Amazon Logo in the Social Auth Login block, which has 9 different styles.
...
User can click on the Amazon logo on the Social Auth Login block
You can also add a button or link anywhere on the site that points
to /user/login/amazon, so theming and customizing the button or link
is very flexible.

When the user opens the /user/login/amazon link, it automatically takes
user to Amazon for authentication. Amazon then returns the user to
Drupal site. If we have an existing Drupal user with the same email address
provided by Amazon, that user is logged in. Otherwise a new Drupal user is
created.
...
https://www.drupal.org/project/issues/social_auth_amazon
